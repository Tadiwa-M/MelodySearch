...
class MetadataSimilarityEngine:
    ...
    def _calculate_metadata_completeness(self, features: Dict[str, Any]) -> float:
        """Rough measure of how complete the metadata is"""
        important_keys = [
            'title', 'artist', 'duration_ms', 'popularity',
            'release_year', 'artist_genres', 'album_type',
            'duration_category', 'era', 'mainstream_level',
            'genre_primary'
        ]
        available = sum(1 for k in important_keys if features.get(k) is not None)
        return available / len(important_keys)

    def _artist_style_similarity(self, artist1, artist2):
        """Compare artist names (string or dict) in a tolerant way."""
        try:
            if isinstance(artist1, dict):
                artist1 = artist1.get("name", "")
            if isinstance(artist2, dict):
                artist2 = artist2.get("name", "")

            if not artist1 or not artist2:
                return 0.3

            return 1.0 if artist1.lower() == artist2.lower() else 0.5
        except Exception:
            return 0.3

    def _title_similarity(self, words1, words2):
        """Compare lists of title keywords."""
        try:
    ...
    def _album_similarity(self, album1, album2):
        """Compare album names (string)."""
        try:
            if not album1 or not album2:
                return 0.3
            return 1.0 if album1.lower() == album2.lower() else 0.4
        except Exception:
            return 0.3

    def _genre_similarity(self, genres1, genres2):
        """Compare genre lists."""
        try:
            if not genres1 or not genres2:
                return 0.3

            set1, set2 = set(map(str.lower, genres1)), set(map(str.lower, genres2))
            overlap = set1.intersection(set2)
    ...
    def _calculate_metadata_completeness(self, song):
        """Calculate how much metadata a song has filled in."""
        try:
            fields = ["title", "artist", "album", "genres"]
            total = len(fields)
            filled = sum(1 for f in fields if song.get(f))
